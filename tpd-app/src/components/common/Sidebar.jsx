...
const NAV = [
  ...
  { to: '/stock-in', icon: TrendingUp, label: 'Stock In' },
  { to: '/stock-out', icon: TrendingDown, label: 'Stock Out' },
...
export default function Sidebar() {
  ...
  const { lowStockProducts, sidebarOpen, setSidebarOpen, refresh } = useApp();
        ...
        {!confirmClear ? (
          ...
          sidebarOpen && (
            <div style={{ padding: '8px 12px', marginBottom: '4px', background: 'rgba(239,68,68,0.1)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(239,68,68,0.25)' }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--accent-red-light)', marginBottom: '8px', fontWeight: 600 }}>Delete all data?</div>
              <div style={{ display: 'flex', gap: '6px' }}>
                <button onClick={handleClearData} style={{ flex: 1, padding: '5px', background: 'var(--accent-red)', border: 'none', borderRadius: 'var(--radius-sm)', color: 'white', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}>Yes, clear</button>
                <button onClick={() => setConfirmClear(false)} style={{ flex: 1, padding: '5px', background: 'var(--bg-input)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-sm)', color: 'var(--text-secondary)', fontSize: '0.75rem', cursor: 'pointer' }}>Cancel</button>
              </div>
            </div>
          )
        )}
